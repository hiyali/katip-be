...
func SendResetPasswordEmail(userEmail string, userName string, token string) (err error) {
	cf := config.Get()
	...
	hermesEmail := hermes.Email{
		Body: hermes.Body{
			Name: userName,
			Intros: []string{
				"You have received this email because a reset password request for " + cf.App.Name + " account was received.",
			},
			Actions: []hermes.Action{
				{
					Instructions: "Click the button below to continue reset password request:",
					Button: hermes.Button{
						Color: "#22BC66",
						Text:  "Reset Password",
						Link:  cf.App.Link + "#/k/reset-password?token=" + token,
					},
				},
			},
			Outros: []string{
				"If you did not request reset password on " + cf.App.Name + ", no further action is required on your part.",
			},
			Signature: "Thanks",
		},
	}
	...
	*/
	...
	header["From"] = cf.Email.Name + "<" + cf.Email.Address + ">"
	header["To"] = userEmail
	header["Subject"] = "You are bing reset password on " + cf.App.Name
	header["Content-Type"] = "text/html; charset=UTF-8"
